typeall=unique(perms([1 1 1 1 2 2 2 2]),'rows');%工件排列次序
time=[];
min_time=10056;%最短加工时间

for typenum =1 :length(typeall)
    % 第i个工件的位置
    A = ones(8,1);
    % 第i个工件加工剩余时间
    T = zeros(8,1);
    ...
    while(sum(A~=12*ones(8,1))~=0) && t<min_time
        % 如果操作臂空闲则进行搜索
        if(S(1)==0||S(2)==0)
            ...
            for i = 1:8
                if(A(i)~=12)
                    k = A(i);
                    [k1, k2] = NextPrecedure(k,typeall(typenum,i));
                    
                    % 防止堵塞
                    num=[0 0 0];%P1-P6总加工工件数量，加工C类数量，加工D类数量
                    if A(i)==2 && i>4
                        for j=1:i-1
                            if 3<A(j) && A(j)<10 && typeall(typenum,j)==1
                                num(1)=num(1)+1;
                                num(2)=num(2)+1;
                            elseif 3<A(j) && A(j)<10 && typeall(typenum,j)==2
                                num(1)=num(1)+1;
                                num(3)=num(3)+1;
                            end
                        end
                        if num(1)==6 || num(2)==4 || num(3)==4
                            k1=k;
                            k2=k;
                        end
                    end
    ...
    time(typenum)=t;
    if t<min_time %更新最小时间及对应操作时刻
        min_time=t;
        min_tt=tt;
        min_s1=s1;
        min_s2=s2;
        min_order=typeall(typenum,:);
    end
end
